...
if st.session_state.drive_service is None:
    ...
    # Huvudfönstret för att visa innehåll
    if st.session_state.drive_service:
        if st.session_state.story_items is None:
            st.info("⬅️ Använd filbläddraren i sidopanelen för att välja en mapp och klicka sedan på 'Läs in filer...'")
        else:
            st.divider()
                    ...
                    with st.container():
                        ...
                        with col1:
                            if item.get('type') == 'image' and item.get('thumbnail'):
                                st.image(item['thumbnail'], width=128)
