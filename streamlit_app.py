...
if st.session_state.drive_service is None:
    ...
    # Allt som inte är i huvudfönstret hamnar nu i sidopanelen
    with st.sidebar:
        ...
        if st.session_state.story_items is not None and st.session_state.organize_mode:
            ...
            #... (Här kommer alla verktyg som Ta bort, Klipp ut, etc.)

        if st.session_state.story_items is not None:
            st.divider()
            st.markdown("### Inställningar & Publicering")
            # ... (Här kommer PDF-inställningar och Skapa-knappen)

    # Huvudfönstret (`col_main` behövs inte längre)
        ...
        if not st.session_state.story_items:
            ...
            for item in st.session_state.story_items:
                with st.container():
                    ...
                    with col1:
                        st.image(item['thumbnail'], width=128) if item.get('type') == 'image' and item.get('thumbnail') else st.markdown("<p style='font-size: 60px;'>📄</p>", unsafe_allow_html=True)
                    with col2:
